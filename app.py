...
def login():
    if request.method == 'POST':
        ...
        if member and member[5] and check_password_hash(member[5], password):
            session["user"] = name
            session["member_id"] = member[1]   # MEMBER_ID
        ...
        if name.lower() == "admin":
            ...
            if name.lower() == "admin":
                ...
                session["is_admin"] = False  # ✅ ADD THIS LINE
                return redirect('/user_billing') 
...
def user_billing():
    ...
    # ✅ ALL BOOKS
    books = conn.execute("""
    SELECT DISTINCT book_id, title, author, category, quantity
    FROM books
""").fetchall()
...
def request_book():
    ...
    existing = conn.execute("""
        SELECT * FROM transactions
        WHERE book_id=? AND member_id=? AND action='Requested'
    """, (book_id, member_id)).fetchone()

    if existing:
        flash("Book already requested!", "warning")
        conn.close()
        return redirect("/user_billing")

    # ✅ INSERT REQUEST
    conn.execute("""
        INSERT INTO transactions (book_id, member_id, action)
        VALUES (?, ?, 'Requested')
    """, (book_id, member_id))
...
def approve(id):
    ...
    # 1. Establish the database connection inside the function
    conn = get_db_connection() 
    ...
    # 3. Fetch the book_id to update the inventory
    # Assuming you need to decrease the quantity of the book associated with this transaction
    req = conn.execute("SELECT book_id FROM transactions WHERE id=?", (id,)).fetchone()
    ...
    # 4. Commit and close MUST be indented inside the function
    conn.commit()
    conn.close()
...
def return_book():
    ...
    # ❌ If no valid record
    if not txn or not txn["due_date"]:
        conn.close()
        flash("No valid borrowed record found!", "danger")
        return redirect("/transactions")
